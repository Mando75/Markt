...
export class TheAppleMarketScenarioDefinition1549593077264
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryBuilder
      .delete()
      .from("scenarios")
      .where("scenario_code = :code", { code: this.scenarioCode })
      .execute();
  }
  ...
  createRoleDist() {
    ...
    const typeCodes = [
      `${this.scenarioCode}_RT_A`,
      `${this.scenarioCode}_RT_B`,
      `${this.scenarioCode}_RT_C`,
      `${this.scenarioCode}_RT_D`,
      `${this.scenarioCode}_RT_E`,
      `${this.scenarioCode}_RT_F`
    ];
    let index = 0;
    for (let i = 0; i < 48; i++) {
      if (index + 1 === typeCodes.length) {
        index = 0;
      }
      roleDist.push(typeCodes[index]);
      index++;
    }
    return roleDist;
  }
